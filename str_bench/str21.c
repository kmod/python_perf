...
PyMODINIT_FUNC PyInit_str21(void) {
    for (int i = 0; i < 20; i++) {
        ...
        for (;;) {
            ...
            {
                {
                    PyObject* v = PyTuple_GET_ITEM(argstuple, 0);
                    //item = _PyObject_Str(v);
                    ...
                    if (v == NULL) {
                        item = PyUnicode_FromString("<NULL>");
                        goto done3;
                    }
                    ...
                    assert(_PyUnicode_CheckConsistency(item, 1));
done3:
                    ;
                }
